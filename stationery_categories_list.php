...
// Get Store name
$stmt_store = $conn->prepare("SELECT name FROM stationery_stores WHERE id = ?");
$stmt_store->bind_param("i", $store_id);
$stmt_store->execute();
$store_result = $stmt_store->get_result();
$store = $store_result->fetch_assoc();
$store_name = $store ? $store['name'] : 'અજાણ્યો સ્ટોર';

// Get all categories for THIS store
$stmt_cat = $conn->prepare("SELECT * FROM stationery_categories WHERE stationery_store_id = ? ORDER BY name");
$stmt_cat->bind_param("i", $store_id);
$stmt_cat->execute();
...
<head>
    ...
    <title><?php echo htmlspecialchars($store_name); ?> - કેટેગરી</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-section h2 { font-size: 2.5rem; font-weight: 700; color: #ffffff; text-shadow: 1px 1px 3px rgba(0,0,0,0.5); margin-bottom: 20px; }
        .search-container { max-width: 600px; margin: 0 auto; position: relative; }
        #searchInput { width: 100%; padding: 15px 20px 15px 50px; border-radius: 50px; border: 1px solid rgba(255,255,255,0.5); font-size: 1.1rem; font-family: 'Noto Sans Gujarati', sans-serif; background: rgba(255, 255, 255, 0.8); backdrop-filter: blur(5px); box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        .search-container i { position: absolute; left: 20px; top: 50%; transform: translateY(-50%); color: var(--secondary-text); }
        ...
        .card { background: var(--card-bg); backdrop-filter: blur(8px); border-radius: 12px; text-decoration: none; color: var(--primary-text); box-shadow: 0 8px 25px rgba(0,0,0,0.15); transition: transform 0.3s ease, box-shadow 0.3s ease; overflow: hidden; border: 1px solid rgba(255, 255, 255, 0.3); display: flex; flex-direction: column; }
        ...
        .card-content { padding: 20px; text-align: center; display: flex; justify-content: space-between; align-items: center; flex-grow: 1; border-top: 1px solid rgba(0,0,0,0.05); }
...
<body>
    <header class="main-header">
        <h1><i class="fa-solid fa-tags"></i> <?php echo htmlspecialchars($store_name); ?></h1>
        <a href="stationery_store.php" class="back-link"><i class="fa-solid fa-arrow-left"></i> બધા સ્ટોર પર પાછા જાઓ</a>
    </header>
    <main>
        <div class="hero-section">
            <h2>તમને કઈ કેટેગરીમાં રસ છે?</h2>
        ...
        <div class="content-container">
            <div class="grid">
                ...
                <a href="stationery_products.php?category_id=<?php echo $category['id']; ?>" class="card">
