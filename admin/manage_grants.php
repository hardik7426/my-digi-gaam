...
if (isset($_POST['save'])) {
    ...
    if (empty($id)) {
        $stmt = $conn->prepare("INSERT INTO grants (project_name, donor_name, amount, year) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssis", $project_name, $donor_name, $amount, $year);
        if ($stmt->execute()) $message = "નવી ગ્રાન્ટ ઉમેરાઈ.";
    } else {
        $stmt = $conn->prepare("UPDATE grants SET project_name=?, donor_name=?, amount=?, year=? WHERE id=?");
        $stmt->bind_param("ssisi", $project_name, $donor_name, $amount, $year, $id);
        if ($stmt->execute()) $message = "ગ્રાન્ટ અપડેટ થઈ.";
    }
    $stmt->close();
}
...
if (isset($_GET['delete'])) {
    ...
    if ($stmt->execute()) {
        $message = "ગ્રાન્ટ સફળતાપૂર્વક દૂર કરાઈ.";
    } else {
...
$edit_grant = ['id' => '', 'project_name' => '', 'donor_name' => '', 'amount' => '', 'year' => ''];
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM grants WHERE id = $id");
    $edit_grant = $result->fetch_assoc();
}
// Fetch all data
...
<head>
    ...
    <title>ગ્રાન્ટ મેનેજ કરો - એડમિન પેનલ</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input[type="text"], input[type="number"] { width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #cbd5e0; border-radius: 8px; box-sizing: border-box; transition: border-color 0.3s, box-shadow 0.3s; font-family: 'Noto Sans Gujarati', sans-serif; font-size: 1rem; }
...
<body>
    <header class="main-header">
        <h1><i class="fa-solid fa-hand-holding-dollar"></i> ગ્રાન્ટ મેનેજ કરો</h1>
    ...
    <main>
        <div class="admin-container">
            <div class="form-section">
                <h3><?php echo empty($edit_grant['id']) ? 'નવી ગ્રાન્ટ ઉમેરો' : 'ગ્રાન્ટ એડિટ કરો'; ?></h3>
                <form action="manage_grants.php" method="post">
                    <input type="hidden" name="id" value="<?php echo htmlspecialchars($edit_grant['id']); ?>">
                    <input type="text" name="project_name" placeholder="પ્રોજેક્ટનું નામ" value="<?php echo htmlspecialchars($edit_grant['project_name']); ?>" required>
                    <input type="text" name="donor_name" placeholder="દાતાનું નામ (વૈકલ્પિક)" value="<?php echo htmlspecialchars($edit_grant['donor_name']); ?>">
                    <input type="number" name="amount" placeholder="રકમ (₹)" value="<?php echo htmlspecialchars($edit_grant['amount']); ?>" required>
                    <input type="text" name="year" placeholder="વર્ષ (દા.ત. 2024-25)" value="<?php echo htmlspecialchars($edit_grant['year']); ?>" required>
            ...
            <div class="table-section">
                ...
                <table>
                    <thead>
                        <tr><th>વર્ષ</th><th>પ્રોજેક્ટનું નામ</th><th>દાતા</th><th>રકમ</th><th>ક્રિયાઓ</th></tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="manage_grants.php?edit=<?php echo $row['id']; ?>">એડિટ</a> |
                                <a href="manage_grants.php?delete=<?php echo $row['id']; ?>" class="delete-action" onclick="return confirm('શું તમે ચોક્કસ આને કાઢી નાખવા માંગો છો?');">ડિલીટ</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    </tbody>
