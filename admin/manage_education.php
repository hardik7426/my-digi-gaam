...
if (isset($_POST['save'])) {
    ...
    if (empty($id)) {
        $stmt = $conn->prepare("INSERT INTO education_content (title, standard, content, pdf_filename) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $title, $standard, $content, $pdf_filename);
        if ($stmt->execute()) $message = "નવી સામગ્રી ઉમેરાઈ.";
    } else {
        $stmt = $conn->prepare("UPDATE education_content SET title=?, standard=?, content=?, pdf_filename=? WHERE id=?");
        $stmt->bind_param("ssssi", $title, $standard, $content, $pdf_filename, $id);
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $result = $conn->query("SELECT pdf_filename FROM education_content WHERE id = $id");
    ...
    if ($stmt->execute()) $message = "સામગ્રી દૂર કરાઈ.";
    $stmt->close();
}
// Fetch data for editing
$edit_content = ['id' => '', 'title' => '', 'standard' => '', 'content' => '', 'pdf_filename' => ''];
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
...
// Fetch all data
$contents = $conn->query("SELECT * FROM education_content ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="gu">
<head>
        ...
        <div class="admin-container">
            <div class="form-section">
                ...
                <form action="manage_education.php" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="<?php echo $edit_content['id']; ?>">
                    <input type="hidden" name="existing_pdf" value="<?php echo $edit_content['pdf_filename']; ?>">
                    
                    <label for="standard">ધોરણ</label>
                    <input type="text" id="standard" name="standard" placeholder="દા.ત. 5, 10, વગેરે" value="<?php echo htmlspecialchars($edit_content['standard']); ?>" required>
                    
                    <label for="title">શીર્ષક</label>
                    <input type="text" id="title" name="title" placeholder="શીર્ષક અહીં લખો" value="<?php echo htmlspecialchars($edit_content['title']); ?>" required>
                    
                    <label for="content">વિગતો</label>
                    <textarea id="content" name="content" placeholder="વિગતો અહીં લખો..." rows="8" required><?php echo htmlspecialchars($edit_content['content']); ?></textarea>
                    
                    <label for="pdf_file">PDF ફાઈલ અપલોડ કરો (વૈકલ્પિક)</label>
                    <input type="file" name="pdf_file" id="pdf_file" accept=".pdf">
                    <?php if (!empty($edit_content['pdf_filename'])): ?>
                        <p class="existing-file-note">હાલની ફાઈલ: <?php echo htmlspecialchars($edit_content['pdf_filename']); ?></p>
                    <?php endif; ?>
            ...
            <div class="table-section">
                ...
                <table>
                    <thead>
                        <tr><th>ધોરણ</th><th>શીર્ષક</th><th>ક્રિયાઓ</th></tr>
                    </thead>
                    <tbody>
