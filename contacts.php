...
if (!empty($search_term)) {
    // The '%' are wildcards, so it matches any part of the name or designation
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        thead { display: none; }
        tbody tr { background: var(--card-bg); backdrop-filter: blur(8px); border-radius: 12px; box-shadow: 0 4px 6px -1px var(--shadow-color); transition: transform 0.2s ease, box-shadow 0.2s ease; }
        ...
        .phone-link { background-color: var(--accent-color-2); color: white; text-decoration: none; padding: 10px 20px; border-radius: 8px; font-weight: 500; display: inline-flex; align-items: center; }
            ...
            <div class="search-container">
                <form action="contacts.php" method="GET">
                    <div class="search-wrapper">
                        ...
                        <input type="text" name="search" id="searchInput" placeholder="નામ અથવા હોદ્દા દ્વારા શોધો..." value="<?php echo htmlspecialchars($search_term); ?>">
                    </div>
                </form>
            </div>
